...
const mongoUri =
  process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/budgetwise';
  ...
  try {
    ...
    if (method === 'email' && gmailUser && gmailPass) {
      try {
        await sendEmail(
          emailValue,
          `Trip invite: ${trip.name} on BudgetWise`,
          `You have been invited to join the trip "${trip.name}" on BudgetWise.\n\nOpen this link: ${inviteLink}\n\nSign in or create an account, then accept the invite.`
        );
      } catch (emailError) {
