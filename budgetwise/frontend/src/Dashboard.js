...
const Dashboard = ({ user, onSignOut, pendingInviteToken, onInviteTokenConsumed }) => {
  ...
  useEffect(() => {
    const storedLocale = window.localStorage.getItem('bw-locale');
    const storedCurrency = window.localStorage.getItem('bw-currency');
    const storedTheme = window.localStorage.getItem('bw-theme');
    const storedSettings = window.localStorage.getItem('bw-settings');
    if (storedLocale) {
  ...
  useEffect(() => {
    window.localStorage.setItem('bw-locale', locale);
  }, [locale]);
  ...
  useEffect(() => {
    window.localStorage.setItem('bw-settings', JSON.stringify(settings));
  }, [settings]);
  ...
  const handleWhatsAppShare = () => {
    ...
    const message = `Join me on BudgetWise for the trip "${trip?.name}"!\n\n${tripInviteLink}`;
