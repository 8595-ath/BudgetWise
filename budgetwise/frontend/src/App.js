...
const App = () => {
  ...
  useEffect(() => {
    const storedToken = window.localStorage.getItem('bw-token');
    if (storedToken) {
    ...
    const storedInviteToken = window.localStorage.getItem('bw-pending-invite') || '';
    if (inviteTokenFromUrl) {
      setPendingInviteToken(inviteTokenFromUrl);
      window.localStorage.setItem('bw-pending-invite', inviteTokenFromUrl);
  ...
  useEffect(() => {
    if (token) {
      ...
      window.localStorage.setItem('bw-token', token);
      axios
      ...
      window.localStorage.removeItem('bw-token');
      setUser(null);
    }
  }, [token]);
  ...
  const handleInviteTokenConsumed = () => {
    ...
    window.localStorage.removeItem('bw-pending-invite');
  };
