...
- Root directory: budgetwise/backend
...
- Root directory: budgetwise/frontend
...
The supplied ZIP intentionally does not include the original `.git` directory or remote URL. After extracting it, create your own repository:

```bash
git init
git add .
git commit -m "Initial BudgetWise version"
git branch -M main
...
- Application branding changed from FinPulse to BudgetWise.
- Browser title, web manifest, email subjects, invite messages, and API service name were updated.
- Source folder renamed from `finpulse` to `budgetwise`.
- Package names changed to `budgetwise-frontend` and `budgetwise-backend`.
- Local storage keys use the `bw-` namespace.
- The default local MongoDB database name is `budgetwise`.
- Real `.env`, `.git`, and `node_modules` content are excluded from the clean ZIP.
